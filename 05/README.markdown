...
Всяка непрекъсната последователност от редове в текста, идентирана с точно четири интервала, ще считате за блок от код и ще ограждате в елементи `<pre>` и `<code>` (`<pre><code>код</code></pre>`). В блоковете от код не се прилага никакво друго форматиране. Всеки символ за whitespace след задължителните четири водещи интервала трябва да се запази както си е. Whitespace в края на редовете, които са блокове код, също се запазва без промяна.
...
Екранацията трябва да се случва без значение къде в текста се намира специалният символ, с малки изключения. Екранация не се прилага над:
